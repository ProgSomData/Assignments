...
type value = 
  | Int of int
  | Closure of string * string list * expr * value env  //Exercise 6.1; updated Closure to store list of parameter  (* (f, xs, fBody, fDeclEnv) *)
...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Letfun(f, xs, fBody, letBody) ->                //Exercise 6.1: updated LetFun
  ...
  and apply fVal xVals =
    ...
    | Closure(f, xs, fBody, fDeclEnv) ->
        ...
        let nv = List.length xVals
        if nx = nv then
            let fBodyEnv = (List.zip xs xVals) @ ((f, fVal) :: fDeclEnv)
            eval fBody fBodyEnv
        elif nv > nx then
            let usedVals, restVals = List.splitAt nx xVals
            let fBodyEnv = (List.zip xs usedVals) @ ((f, fVal) :: fDeclEnv)
            apply (eval fBody fBodyEnv) restVals
        else
            failwith "eval Call: too few arguments (partial application not supported)"
