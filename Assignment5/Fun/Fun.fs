...
type value = 
  | Int of int
  //Exercise 4.3
  //we modified closure to take a list of xs instead of x
...
let rec eval (e : expr) (env : value env) : int =
    ...
    | If(e1, e2, e3) -> 
      ...
      //Exercise 4.3
      //We change the x to xs to reflect the change we made in Absyn.fs and Closure
    ...
    | Call (Var f , eArgs) ->
      ...
      | Closure (f, xs, fBody, fDeclEnv) ->
        let xVal = List.map (fun x -> Int (eval x env)) eArgs
        let parameterEnv  = List.zip xs xVal
        let fBodyEnv = parameterEnv @ ((f, fClosure) :: fDeclEnv)
        eval fBody fBodyEnv
